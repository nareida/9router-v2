...
export async function GET(req, res) {
  try {
    ...
    if (!installed) {
      return res.json({ installed: false, settings: null, message: "Hermes Agent is not installed" });
    }
    ...
    return res.status(500).json({ error: "Failed to check hermes settings" });
  }
}
...
export async function POST_handler(req, res) {
  try {
    ...
    return res.status(500).json({ error: "Failed to update hermes settings" });
  }
}
...
export async function DELETE(req, res) {
  try {
    ...
    return res.json({ success: true, message: `${PROVIDER_NAME} model block removed` });
  } catch (error) {
    console.log("Error resetting hermes settings:", error);
    return res.status(500).json({ error: "Failed to reset hermes settings" });
  }
}
