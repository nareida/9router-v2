...
export async function GET(req, res) {
  try {
    ...
    return res.status(500).json({ error: "Failed to check openclaw settings" });
  }
}
...
export async function POST_handler(req, res) {
  try {
    ...
    return res.status(500).json({ error: "Failed to update openclaw settings" });
  }
}
...
export async function DELETE(req, res) {
  try {
    ...
    return res.status(500).json({ error: "Failed to reset openclaw settings" });
  }
}
