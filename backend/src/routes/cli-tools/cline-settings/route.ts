...
export async function GET(req, res) {
  try {
    ...
    if (!installed) {
      return res.json({ installed: false, settings: null, message: "Cline CLI is not installed" });
    }
    ...
    return res.status(500).json({ error: "Failed to check cline settings" });
  }
}
...
export async function POST_handler(req, res) {
  try {
    ...
    return res.json({ success: true, message: "Cline settings applied successfully!", globalStatePath: getGlobalStatePath() });
  } catch (error) {
    console.log("Error updating cline settings:", error);
    return res.status(500).json({ error: "Failed to update cline settings" });
  }
}
...
export async function DELETE(req, res) {
  try {
    const globalState = await readJson(getGlobalStatePath());
    if (!globalState) {
      return res.json({ success: true, message: "No settings file to reset" });
    }
    ...
    return res.json({ success: true, message: "9Router settings removed from Cline" });
  } catch (error) {
    console.log("Error resetting cline settings:", error);
    return res.status(500).json({ error: "Failed to reset cline settings" });
  }
}
