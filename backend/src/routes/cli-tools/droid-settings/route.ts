...
export async function GET(req, res) {
  try {
    ...
    return res.status(500).json({ error: "Failed to check droid settings" });
  }
}
...
export async function POST_handler(req, res) {
  try {
    ...
    return res.status(500).json({ error: "Failed to update droid settings" });
  }
}
...
export async function DELETE(req, res) {
  try {
    ...
    return res.status(500).json({ error: "Failed to reset droid settings" });
  }
}