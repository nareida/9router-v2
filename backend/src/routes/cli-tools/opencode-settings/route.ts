...
export async function GET(req, res) {
  try {
    ...
    return res.status(500).json({ error: "Failed to check opencode settings" });
  }
}
...
export async function POST_handler(req, res) {
  try {
    ...
    if (!baseUrl || modelsArray.length === 0) {
      return res.status(400).json({ error: "baseUrl and at least one model are required" });
    }
    ...
    return res.status(500).json({ error: "Failed to apply settings" });
  }
}
  ...
  try {
    ...
    try {
      ...
      if (error.code === "ENOENT") {
        return res.json({ success: true, message: "No config file found" });
      }
      throw error;
    }
    ...
    return res.status(500).json({ error: "Failed to patch settings" });
  }
}
...
export async function DELETE_handler(req, res) {
  try {
    ...
    return res.status(500).json({ error: "Failed to reset opencode settings" });
  }
}
