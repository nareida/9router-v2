...
import { DEFAULT_PLUGINS, LOCAL_STDIO_PLUGINS, ALLOWED_MCP_COMMANDS, buildManagedMcpServers } from "../../../shared/constants/coworkPlugins.js";
...
export async function GET(req, res) {
  try {
    ...
    if (!installed) {
      return res.json({ installed: false, config: null, message: "Claude Desktop (Cowork mode) not detected" });
    }
    ...
    return res.status(500).json({ error: "Failed to read cowork settings" });
  }
}
...
export async function POST_handler(req, res) {
  ...
  try {
    ...
    return res.status(500).json({ error: "Failed to apply cowork settings" });
  }
}
...
export async function DELETE(req, res) {
  try {
    const meta = await readJson(await getMetaPath());
    if (!meta?.appliedId) {
      return res.json({ success: true, message: "No active config to reset" });
    }
    ...
    try { await cleanup1pLegacy(); } catch { /* ignore */ }
    return res.json({ success: true, message: "Cowork config reset" });
  } catch (error) {
    console.log("Error resetting cowork settings:", error);
    return res.status(500).json({ error: "Failed to reset cowork settings" });
  }
}
