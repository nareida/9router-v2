...
export async function GET(req, res) {
  try {
    ...
    if (!installed) {
      return res.json({ installed: false, settings: null, message: "Kilo Code CLI is not installed" });
    }
    ...
    return res.status(500).json({ error: "Failed to check kilo settings" });
  }
}
...
export async function POST_handler(req, res) {
  try {
    ...
    return res.json({ success: true, message: "Kilo Code settings applied successfully!", authPath: getAuthPath() });
  } catch (error) {
    console.log("Error updating kilo settings:", error);
    return res.status(500).json({ error: "Failed to update kilo settings" });
  }
}
...
export async function DELETE(req, res) {
  try {
    ...
    return res.json({ success: true, message: "9Router settings removed from Kilo Code" });
  } catch (error) {
    console.log("Error resetting kilo settings:", error);
    return res.status(500).json({ error: "Failed to reset kilo settings" });
  }
}
