...
export function getClientIp(request) {
  const xff = request.headers?.["x-forwarded-for"] ?? request.headers?.get?.("x-forwarded-for");
  if (xff) return xff.split(",")[0].trim();
  return request.headers?.["x-real-ip"] ?? request.headers?.get?.("x-real-ip") ?? request.ip ?? "unknown";
}
