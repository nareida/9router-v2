...
function RouterNode({ data }) {
  return (
    <div className="flex items-center justify-center px-5 py-3 rounded-xl border-2 border-primary bg-primary/5 shadow-md min-w-[130px]">
      ...
      <img src="/logo_v2.png" alt="9Router" className="w-6 h-6 mr-2" />
...
export default function ProviderTopology({ providers = [], activeRequests = [], lastProvider = "", errorProvider = "" }) {
  ...
  const activeKey = useMemo(
    () => activeRequests.map((r) => {
      ...
      return p === "cb" ? "codebuddy" : p;
    }).filter(Boolean).sort().join(","),
    [activeRequests]
  );
  const lastKey = lastProvider?.toLowerCase() === "cb" ? "codebuddy" : (lastProvider?.toLowerCase() || "");
  const errorKey = errorProvider?.toLowerCase() === "cb" ? "codebuddy" : (errorProvider?.toLowerCase() || "");
