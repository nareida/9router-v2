        ...
        <table className="w-full table-fixed text-left">
          <tbody>
            {currentPageRows.map((quota) => {
              ...
              const isTokens = quota.name?.toLowerCase() === "tokens";
              const displayUsed = isTokens ? Math.max(0, quota.total - quota.used) : quota.used;
                    ...
                    <div className={compact ? "space-y-1" : "space-y-1.5"}>
                      ...
                      <div className={`flex items-center justify-between ${compact ? "text-[10px]" : "text-xs"}`}>
                        <span className="text-text-muted">
                          {displayUsed.toLocaleString()} / {quota.total > 0 ? quota.total.toLocaleString() : "∞"}
                        </span>
