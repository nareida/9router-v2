...
export default function QuotaProgressBar({
  ...
  const isTokens = label?.toLowerCase() === "tokens";
  const displayUsed = isTokens ? Math.max(0, total - used) : used;
  const displayUnit = isTokens ? (unit === "requests" ? "tokens" : unit) : unit;
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex items-center justify-between text-xs text-text-muted">
        <span>
          {displayUsed.toLocaleString()} / {total.toLocaleString()} {displayUnit || "requests"}
        </span>
        {countdown !== "-" && (
