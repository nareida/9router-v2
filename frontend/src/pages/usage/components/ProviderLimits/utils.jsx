import { getModelsByProviderId } from "@/shared/config/providerModels.js";
    ...
    switch (provider.toLowerCase()) {
      ...
      case "leonardo":
        if (data.quotas) {
          Object.entries(data.quotas).forEach(([name, quota]) => {
            normalizedQuotas.push({
              name: name === "tokens" ? "Tokens" : name,
              used: quota.used || 0,
              total: quota.total || 0,
              resetAt: quota.resetAt || null,
              unit: quota.unit || null,
              remaining: quota.remaining !== undefined ? quota.remaining : null,
            });
          });
        }
        break;
        ...
        if (data.quotas) {
          Object.entries(data.quotas).forEach(([name, quota]) => {
            normalizedQuotas.push({
              ...
              // Don't forward 'remaining' — it's absolute neurons, not a percentage.
              // getRemainingPercentage() would interpret 10000 as 10000%. Use used/total.
            });
          });
        }
        break;
