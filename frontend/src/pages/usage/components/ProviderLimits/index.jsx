...
const QUOTA_SORT_OPTIONS = [
  ...
  { value: "remaining-asc", label: "Least credits first" },
  { value: "remaining-desc", label: "Most credits first" },
  { value: "balance-asc", label: "Saldo Terendah (Lowest Balance)" },
  { value: "balance-desc", label: "Saldo Tertinggi (Highest Balance)" },
];
...
export default function ProviderLimits() {
  ...
  const fetchConnections = useCallback(
    async (targetPage = page) => {
      try {
        // Map quotaSortMode to server-side sort param
        let serverSort = "priority";
        if (expiringFirst) serverSort = "expiring";
        else if (quotaSortMode === "remaining-asc") serverSort = "usage-asc";
        else if (quotaSortMode === "remaining-desc") serverSort = "usage-desc";
        else if (quotaSortMode === "balance-asc") serverSort = "balance-asc";
        else if (quotaSortMode === "balance-desc") serverSort = "balance-desc";
    ...
    [accountFilter, expiringFirst, page, pageSize, providerFilter, quotaSortMode],
  );
  ...
  useEffect(() => {
    ...
    initializeData();
  }, [fetchConnections, fetchQuota, page, hasHydrated]);

  const PROVIDER_FILTER_KEY = "quotaProviderFilter";
  const ACCOUNT_FILTER_KEY = "quotaAccountFilter";
  const QUOTA_SORT_KEY = "quotaSortMode";
  ...
  useEffect(() => {
    ...
    const storedProvider = window.localStorage.getItem(PROVIDER_FILTER_KEY);
    if (storedProvider) setProviderFilter(storedProvider);

    const storedAccount = window.localStorage.getItem(ACCOUNT_FILTER_KEY);
    if (storedAccount) setAccountFilter(storedAccount);

    const storedSort = window.localStorage.getItem(QUOTA_SORT_KEY);
    if (storedSort) setQuotaSortMode(storedSort);
  ...
  }, [autoRefresh, hasHydrated]);

  // Persist filters
  useEffect(() => {
    if (typeof window === "undefined" || !hasHydrated) return;
    window.localStorage.setItem(PROVIDER_FILTER_KEY, providerFilter);
  }, [providerFilter, hasHydrated]);

  useEffect(() => {
    if (typeof window === "undefined" || !hasHydrated) return;
    window.localStorage.setItem(ACCOUNT_FILTER_KEY, accountFilter);
  }, [accountFilter, hasHydrated]);

  useEffect(() => {
    if (typeof window === "undefined" || !hasHydrated) return;
    window.localStorage.setItem(QUOTA_SORT_KEY, quotaSortMode);
  }, [quotaSortMode, hasHydrated]);

  // Auto-refresh interval
  useEffect(() => {
    if (!hasHydrated || !autoRefresh) {
  ...
  }, [autoRefresh, refreshAll, hasHydrated]);
  ...
  const isConnectionExhausted = (conn) => {
    const quota = quotaData[conn.id];
    return quota && quota.message === "Account disabled (Credits exhausted)";
  };
  ...
  const handleDisableExhausted = () => {
    const ids = sortedConnections
      .filter((c) => (c.isActive ?? true) && isConnectionExhausted(c))
      .map((c) => c.id);
    bulkSetActive(ids, false);
  };
  ...
  const handleEnableAvailable = () => {
    const ids = sortedConnections
      .filter((c) => !(c.isActive ?? true) && !isConnectionDepleted(c) && !isConnectionExhausted(c))
      .map((c) => c.id);
    bulkSetActive(ids, true);
  };

  const { totalCredits, providerStats, exhaustedCount } = useMemo(() => {
    let total = 0;
    const stats = {};
    let exhausted = 0;
    ...
    sortedConnections.forEach((conn) => {
      ...
      if (entry && entry.message === "Account disabled (Credits exhausted)") {
        exhausted += 1;
      }
      ...
      if (!stats[provider]) stats[provider] = { accounts: 0, credits: 0 };
      stats[provider].accounts += 1;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        {/* Totals Display */}
        <div className="flex flex-wrap items-center gap-2 text-sm font-medium text-text-primary px-1">
          <div className="flex items-center gap-1.5 bg-black/5 dark:bg-white/10 px-2.5 py-1 rounded-md" title="Total active accounts on this page">
            <span className="material-symbols-outlined text-[16px] text-text-muted">group</span>
            <span>{totals.providerFilteredConnections} Accounts</span>
          </div>
          <div className="flex items-center gap-1.5 bg-primary/10 px-2.5 py-1 rounded-md" title="Total credits across these accounts">
            <span className="material-symbols-outlined text-[16px] text-primary">database</span>
            <span className="text-primary">{Math.floor(totalCredits)} Credits</span>
          </div>
          {exhaustedCount > 0 && (
            <div className="flex items-center gap-1.5 bg-red-500/10 px-2.5 py-1 rounded-md" title="Total exhausted accounts">
              <span className="material-symbols-outlined text-[16px] text-red-500">warning</span>
              <span className="text-red-500">{exhaustedCount} Exhausted</span>
            </div>
          )}
          ...
          <div className="flex flex-wrap items-center gap-1 ml-2">
            {Object.entries(providerStats).map(([provider, stats]) => (
              <div key={provider} className="flex items-center gap-1 bg-black/5 dark:bg-white/5 px-2 py-0.5 rounded text-xs text-text-muted border border-black/10 dark:border-white/10" title={`${stats.accounts} accounts`}>
                <span className="capitalize">{provider}</span>: <span className="font-semibold text-text-primary">{Math.floor(stats.credits)}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-1.5 justify-end">
          ...
          <select
            ...
            className="h-8 rounded-lg border border-black/10 bg-black/[0.02] px-2 text-xs text-text-primary outline-none transition-colors hover:bg-black/5 dark:border-white/10 dark:bg-white/[0.03] dark:hover:bg-white/10"
            aria-label="Sort quotas by remaining"
          >
          ...
          {/* Bulk: disable exhausted */}
          <button
            type="button"
            onClick={handleDisableExhausted}
            disabled={bulkToggling}
            className="flex h-8 shrink-0 items-center gap-1 rounded-lg border border-red-500/30 px-2 text-xs text-red-500 transition-colors hover:bg-red-500/10 disabled:opacity-50"
            title="Disable connections that are exhausted"
          >
            <span className="material-symbols-outlined text-[14px]">block</span>
            <span className="hidden sm:inline">Disable Exhausted</span>
          </button>
      ...
      {(expiringFirst || quotaSortMode !== "default") && (
        <div className="rounded-xl border border-amber-500/20 bg-amber-500/10 px-3 py-2 text-xs text-amber-700 dark:text-amber-300 mb-3">
          Sorting currently reorders accounts inside the current page.
          Cross-page ordering still follows backend pagination. Try increasing the page size to sort across more accounts.
        </div>
      )}
                ...
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2 min-w-0">
                    <div className="w-8 h-8 shrink-0 rounded-md flex items-center justify-center overflow-hidden">
                      <ProviderIcon
                        src={conn.provider === "codebuddy" || conn.provider === "cb" ? "/providers/codebuddy.svg" : `/providers/${conn.provider}.png`}
                        alt={conn.provider}
                        size={32}
