...
import ModelsCard from "@/pages/providers/components/ModelsCard";
import { TTS_PROVIDER_CONFIG } from "@/shared/constants/ttsProviders";
import { getTtsVoicesForModel } from "@/shared/config/ttsModels.js";
...
const KIND_EXAMPLE_CONFIG = {
  ...
  video: {
    ...
    defaultResponse: `{\n  "data": [\n    { "url": "..." }\n  ]\n}`,
    extraFields: [
      { key: "duration", label: "Duration (seconds)", type: "number", default: 5, min: 1, max: 10 },
      { key: "resolution", label: "Resolution", type: "select", default: "720", options: ["480", "720", "1080"] },
      { key: "aspect_ratio", label: "Aspect Ratio", type: "select", default: "16:9", options: ["16:9", "9:16", "1:1"] },
      { key: "image_url", label: "Image Reference (URL)", type: "text", default: "", placeholder: "https://example.com/image.png" },
      { key: "video_url", label: "Video Reference (URL)", type: "text", default: "", placeholder: "https://example.com/video.mp4" },
      { key: "motion_strength", label: "Motion Strength", type: "number", default: 5, min: 1, max: 10 },
    ],
  },
  music: {
    inputLabel: "Prompt",
...
function GenericExampleCard({ providerId, kind }) {
  ...
  useEffect(() => {
    ...
    setExtraValues((prev) => {
      const next = { ...prev };
      let changed = false;
      (safeExConfig.extraFields || []).forEach((f) => {
        const paramDef = Array.isArray(selectedModelObj?.paramDefs)
          ? selectedModelObj.paramDefs.find((p) => p.name === f.key)
          : null;
        const validValues = paramDef?.validValues || f.options || [];
        const currentVal = prev[f.key];
        
        // If current value is invalid for the selected model, reset to the default or first valid value
        if (currentVal && validValues.length > 0 && !validValues.includes(currentVal)) {
          const newVal = paramDef?.default ?? validValues[0] ?? "";
  ...
  }, [selectedModel, selectedModelObj, safeExConfig.extraFields]);
  ...
  return (
    <Card>
      ...
      <div className="flex flex-col gap-2.5">
        ...
        {/* Friendly labels for Weavy size options (ratio strings from validValues) */}
        {(() => {
          const WEAVY_SIZE_LABELS = {
            "1:1":  "1:1  Square",
            "9:16": "9:16  Portrait",
            "3:4":  "3:4  Portrait",
            "16:9": "16:9  Landscape",
          ...
          return (exConfig.extraFields || [])
            .filter((f) => kindModels.length === 0 || (Array.isArray(selectedModelObj?.params) && selectedModelObj.params.includes(f.key)))
            .map((f) => {
              const paramDef = Array.isArray(selectedModelObj?.paramDefs)
                ? selectedModelObj.paramDefs.find((p) => p.name === f.key)
                : null;
              const options = paramDef?.validValues || f.options || [];
              // Use friendly labels if all options are ratio strings covered by WEAVY_SIZE_LABELS
              const isWeavySizeField = options.length > 0 && options.every((o) => o in WEAVY_SIZE_LABELS);
                    ...
                    >
                      {(options || []).map((opt) => (
                        <option key={opt} value={opt}>
                          {opt === "" ? "(default)" : isWeavySizeField ? (WEAVY_SIZE_LABELS[opt] || opt) : opt}
                        </option>
                      ))}
                  </select>
                ) : f.type === "text" ? (
        ...
        <div>
          ...
          {kind === "video" && result?.data?.data?.length > 0 && (() => {
            ...
            // Weavy returns [PNG, PNG, MP4] — find the actual video URL
            const videoItem =
              dataItems.find((d) => /\.mp4(\?|$)/i.test(d.url || "") || /\/video\//i.test(d.url || "")) ||
              dataItems[dataItems.length - 1];
            const videoUrl = videoItem?.url;
            if (!videoUrl) return null;
            const proxiedUrl = `/api/media-proxy?url=${encodeURIComponent(videoUrl)}`;
            return (
              <div className="mt-2">
                <div className="flex items-center justify-end mb-1.5">
                  <a
